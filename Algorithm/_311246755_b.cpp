#include "_311246755_b.h"


REGISTER_ALGORITHM(_311246755_b)
...
//Constructors
_311246755_b::MazeCell::MazeCell(MazeObstacle _obstacle) : hasBookmark(false), numOfDirsTried(0), obstacle(_obstacle) {
...
_311246755_b::MazeCell::MazeCell() : hasBookmark(false), numOfDirsTried(0), obstacle(MazeObstacle::UNKNOWN) {
	for (int i = 0; i < 4; i++) {
		triedDirection[Move::UP] = false;
		triedDirection[Move::DOWN] = false;
		triedDirection[Move::LEFT] = false;
 ...
 */
_311246755_b::MazeCell::MazeCell(const MazeCell& copied) {
 ...
 */
void _311246755_b::undoMove(const Move dir) {
	switch (dir) {
	case Move::LEFT:
 ...
 */
std::pair <int, int> _311246755_b::calcNextLocation(const Move move) {
	int nextRow = curRow;
	int nextCol = curCol;
	switch (move) {
	case Move::UP:
 ...
 */
void _311246755_b::updateLocation(const Move move) {
 ...
 */
void _311246755_b::addNewCell(const int row, const int col, const MazeObstacle obstacle) {
 ...
 */
void _311246755_b::addNewCell(const int row, const int col, MazeCell & cell) {
 ...
 */
void _311246755_b::updateCellObstacle(const int row, const int col, MazeObstacle newObstacle) {
 ...
 */
void _311246755_b::updateTriedFromOrigin(_311246755_b::MazeCell & cell) {
	if (!path.empty()) {
		switch (path.top()) {
		case Move::LEFT:
 ...
 */
AbstractAlgorithm::Move _311246755_b::backtrackMove() {
 ...
 */
AbstractAlgorithm::Move _311246755_b::chooseMove() {
	...
	else if (cell.numOfDirsTried < 4) { //choose a direction that wasn't tried yet
		isBacktracking = false;
		isDirectionChosen = true;
		nextMove = chooseUntriedDirection(cell);
	}
 ...
 */
void _311246755_b::updateKnownDimentions(Move move) {
	switch (move) {
	case Move::UP:
 ...
 */
int _311246755_b::setHeightBounds(int seq) {
 ...
 */
int _311246755_b::setWidthBounds(int seq) {
 ...
 */
void _311246755_b::unifyCells(MazeCell & receiver, MazeCell & giver) {
 ...
 */
void _311246755_b::unifyRows(std::map <int, MazeCell> receiver, std::map <int, MazeCell> giver, const int rowNumOfReceiver) {
 ...
 */
int _311246755_b::findCorrectCoord(const int oldCoord, const int thresh, const int mazeDim) {
	int res = oldCoord;
	while (res < thresh) {
		res += mazeDim;
	}
